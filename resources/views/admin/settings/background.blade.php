  ...
  <section class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <form method="POST" action="{{ route('admin.settings.background.update') }}" id="settings-form" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          @csrf
          <input type="hidden" name="bg_zone0_blur"    id="zone0-blur-hidden"    value="{{ $settings['bg_zone0_blur'] ?? 0 }}">
          <input type="hidden" name="bg_zone0_opacity" id="zone0-opacity-hidden" value="{{ $settings['bg_zone0_opacity'] ?? 0.5 }}">
        ...
        <!-- == ZONE 0 == -->
        <h2 class="text-xl font-bold mb-4">Zone 0 – Ảnh nền Menu Bar (Dưới thanh Navbar)</h2>
        ...
        <div class="flex flex-col md:flex-row gap-8">
          ...
          <div class="flex-1 space-y-4">
            ...
            <div>
                ...
                <label for="zone0-image-input" id="zone0-dropzone" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700/50 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 transition-all duration-200 group">
                    ...
                    <input id="zone0-image-input" type="file" class="hidden" accept="image/*" />
                </label>
                ...
                <div class="flex items-center justify-between mt-3">
                    <div class="flex items-center gap-2 overflow-hidden">
                        ...
                        <p id="zone0-file-name" class="text-sm text-gray-600 dark:text-gray-300 truncate">
                            {{ !empty($settings['bg_zone0_image']) ? basename($settings['bg_zone0_image']) : 'Chưa có ảnh được chọn' }}
                        </p>
                    </div>
                    <div class="flex gap-2 ml-2">
                        <button id="zone0-upload-btn" class="btn btn-sm btn-primary shadow-sm" disabled>Tải lên</button>
                        <button type="button" id="zone0-delete-btn" class="btn btn-sm btn-error shadow-sm {{ empty($settings['bg_zone0_image']) ? 'hidden' : '' }}">Xóa ảnh</button>
                    </div>
                </div>
                <p id="zone0-current-path" class="hidden">{{ $settings['bg_zone0_image'] ?? '' }}</p>
            </div>

            <!-- Blur slider -->
            <div>
                <label class="block font-medium text-sm text-gray-700 dark:text-gray-300">Độ mờ (Blur): <span id="zone0-blur-value">{{ $settings['bg_zone0_blur'] ?? 0 }}</span>px</label>
                <input type="range" id="zone0-blur-slider" min="0" max="30" value="{{ $settings['bg_zone0_blur'] ?? 0 }}" class="range mt-1 w-full max-w-xs">
            </div>

            <!-- Opacity slider -->
            <div>
                <label class="block font-medium text-sm text-gray-700 dark:text-gray-300">Độ trong suốt (Opacity): <span id="zone0-opacity-value">{{ $settings['bg_zone0_opacity'] ?? 0.5 }}</span></label>
                <input type="range" id="zone0-opacity-slider" min="0" max="1" step="0.05" value="{{ $settings['bg_zone0_opacity'] ?? 0.5 }}" class="range mt-1 w-full max-w-xs">
            </div>
          ...
          <!-- Right: Live Preview -->
          <div class="flex-1">
            <label class="block font-medium text-sm text-gray-700 dark:text-gray-300 mb-1">Xem trước Zone 0</label>
            <div id="zone0-preview" class="w-full h-64 rounded-lg overflow-hidden relative bg-gray-200 dark:bg-gray-700">
                <div id="zone0-preview-inner"
                     class="absolute -inset-10 bg-center transition-all"
                     style="
                         background-size: cover;
                         background-repeat: no-repeat;
                         background-image: url('{{ !empty($settings['bg_zone0_image']) ? asset($settings['bg_zone0_image']) : '' }}');
                         filter: blur({{ $settings['bg_zone0_blur'] ?? 0 }}px);
                         opacity: {{ $settings['bg_zone0_opacity'] ?? 0.5 }};
                     ">
                </div>
            </div>
          </div>
