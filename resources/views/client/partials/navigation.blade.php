<div class="bg-base-100/70 navbar sticky top-0 z-50 shadow-sm backdrop-blur-md">
    <div class="navbar-start">
        <!-- Mobile dropdown -->
        <div class="dropdown">
            <label class="btn btn-ghost lg:hidden" tabindex="0" role="button">
                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                </svg>
            </label>
            <ul class="dropdown-content menu menu-sm z-10 mt-3 max-h-[86vh] w-max overflow-auto rounded-box bg-base-100 p-2 shadow" tabindex="0" style="flex-flow: column;">
                <li><a class="font-semibold" href="/">Trang chủ</a></li>
                <li><a class="font-semibold" href="{{ route('client.shop.index') }}">Mới nhất</a></li>
        ...
        <a class="btn btn-ghost swap swap-flip h-20 w-20 px-0 md:h-24 md:w-24" href="/">
            <img class="h-18 w-18 swap-off absolute text-primary md:h-24 md:w-24 lg:h-24 lg:w-24 xl:h-24 xl:w-24" src="{{ asset('images/logos/logo-nav.png') }}" alt="Logo">
            <img class="h-18 w-18 swap-on absolute text-primary md:h-24 md:w-24 lg:h-24 lg:w-24 xl:h-24 xl:w-24" src="{{ asset('images/logos/logo-nav.png') }}" alt="Logo">
        </a>
        <a class="btn btn-ghost px-0" href="/">
            <span class="ml-1 hidden text-xl font-semibold md:block" style="text-shadow: 0.1px 0.1px 0 #000, -0.1px -0.1px 0 #000, 0.1px -0.1px 0 #000, -0.1px 0.1px 0 #000">
                THIETKEDECOR.VN</span>
        </a>
    </div>

    <div class="navbar-center hidden lg:flex">
        <ul class="dropdown-hover-container menu menu-horizontal px-1 text-base">
            <li><a href="/">TRANG CHỦ</a></li>
            <li><a href="{{ route('client.shop.index') }}">MỚI NHẤT</a></li>
            @foreach ($shared_categories as $shared_category)
                <li class="dropdown-hover">
                    <details class="dropdown">
                        <summary class="mx-0">{{ strtoupper($shared_category->name) }}</summary>
                        <ul class="dropdown-content menu z-[1] w-52 rounded-box bg-base-100 p-2 shadow" style="margin-top:0;">
                            <li><a class="text-lg font-bold" href="{{ route('client.shop.category', ['slug' => $shared_category->slug]) }}">{{ strtoupper($shared_category->name) }}</a></li>
    ...
    <div class="navbar-end">
        <label class="toggle text-base-content">
            <input class="theme-controller hidden" type="checkbox" value="dark">
