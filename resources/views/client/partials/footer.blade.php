<footer class="footer sm:footer-horizontal bg-base-300 text-base-content p-10">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 w-full gap-8 container mx-auto">
        ...
        <nav class="flex flex-col">
            ...
            <div class="flex flex-col">
                <a href="{{ route('client.contact.index') }}" class="link link-hover">Zalo/Call: 0908556913 (Minh)</a>
                <a href="{{ route('client.contact.index') }}" class="link link-hover">Zalo/Call: 0975038534 (Phương)</a>
            </div>
        ...
        <nav class="flex flex-col">
            ...
            @foreach ($shared_categories->take(8) as $shared_category)
                <a href="{{ route('client.shop.category', ['slug' => $shared_category->slug]) }}"
                    class="link link-hover">- {{ $shared_category->name }}</a>
            @endforeach
        </nav>
        <nav class="flex flex-col">
